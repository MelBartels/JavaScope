...
public class cmd_scope_guide_on extends cmdScopeBase implements cmdScope {
    ...
    public boolean process(cmdScopeList cl) {
        super.process(cl);
        if (!cl.t.guideActive && cfg.getInstance().initialized()) {
            if (cl.t.HandpadModes != null)
                cl.t.HandpadModes.handpadModeBeep();
            if (cfg.getInstance().handpadUpdateDrift)
                cl.t.startDriftSidT = astroTime.getInstance().sidT.rad;
            cl.t.initAllGuide();
            return true;
        }
        return false;
